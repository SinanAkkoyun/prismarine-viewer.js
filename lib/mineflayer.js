const EventEmitter = require('events')
const { WorldView } = require('../viewer')

module.exports = (bot, { viewDistance = 6, firstPerson = false, port = 3000, prefix = '', proxyPort = 3001 }) => {
  (async () => {
    const cors = require('cors')
    const express = require('express')
    const app = express();

    app.use(cors())
    
    app.get('/profile/:username', async (req, res) => {
      try {
        const username = req.params.username;
        const uuidResponse = await fetch(`https://api.mojang.com/users/profiles/minecraft/${username}`);
        
        if (!uuidResponse.ok) {
          res.status(uuidResponse.status).json({ error: 'Failed to fetch UUID' });
          return;
        }
    
        const uuidData = await uuidResponse.json();
        const uuid = uuidData.id;
    
        const skinResponse = await fetch(`https://sessionserver.mojang.com/session/minecraft/profile/${uuid}`);
        
        if (!skinResponse.ok) {
          res.status(skinResponse.status).json({ error: 'Failed to fetch skin' });
          return;
        }
    
        const skinData = await skinResponse.json();
        const properties = skinData.properties.find(prop => prop.name === 'textures');
        const textureData = JSON.parse(Buffer.from(properties.value, 'base64').toString());
        const skinUrl = textureData.textures.SKIN.url;
    
        res.json({ uuid, skinUrl });
      } catch (error) {
        res.status(500).json({ error: 'Server error' });
      }
    })
    // Endpoint to get UUID by username
    app.get('/uuid/:username', async (req, res) => {
      try {
        const username = req.params.username;
        const response = await fetch(`https://api.mojang.com/users/profiles/minecraft/${username}`);
        
        if (!response.ok) {
          res.status(response.status).json({ error: 'Failed to fetch UUID' });
          return;
        }

        const data = await response.json();
        res.json(data); // Send UUID data back to client
      } catch (error) {
        res.status(500).json({ error: 'Server error' });
      }
    });

    // Endpoint to get skin URL by UUID
    app.get('/skin/:uuid', async (req, res) => {
      try {
        const uuid = req.params.uuid;
        const response = await fetch(`https://sessionserver.mojang.com/session/minecraft/profile/${uuid}`);
        
        if (!response.ok) {
          res.status(response.status).json({ error: 'Failed to fetch skin' });
          return;
        }

        const data = await response.json();
        const properties = data.properties.find(prop => prop.name === 'textures');
        const textureData = JSON.parse(Buffer.from(properties.value, 'base64').toString());
        const skinUrl = textureData.textures.SKIN.url; // Extract skin URL
        res.json({ skinUrl });
      } catch (error) {
        res.status(500).json({ error: 'Server error' });
      }
    });


    app.listen(proxyPort, () => {
      // console.log(`Server is running on port ${proxyPort}`);
    })
  })()

  const express = require('express')

  const app = express()
  const http = require('http').createServer(app)

  const io = require('socket.io')(http, { path: prefix + '/socket.io' })

  let worldView

  const { setupRoutes } = require('./common')
  setupRoutes(app, prefix)

  const sockets = []
  const primitives = {}

  bot.viewer = new EventEmitter()

  bot.viewer.erase = (id) => {
    delete primitives[id]
    for (const socket of sockets) {
      socket.emit('primitive', { id })
    }
  }

  bot.viewer.drawBoxGrid = (id, start, end, color = 'aqua') => {
    primitives[id] = { type: 'boxgrid', id, start, end, color }
    for (const socket of sockets) {
      socket.emit('primitive', primitives[id])
    }
  }

  bot.viewer.drawLine = (id, points, color = 0xff0000) => {
    primitives[id] = { type: 'line', id, points, color }
    for (const socket of sockets) {
      socket.emit('primitive', primitives[id])
    }
  }

  bot.viewer.drawPoints = (id, points, color = 0xff0000, size = 5) => {
    primitives[id] = { type: 'points', id, points, color, size }
    for (const socket of sockets) {
      socket.emit('primitive', primitives[id])
    }
  }

  bot.viewer.reset = () => {
    for (const socket of sockets) {
      socket.emit('version', bot.version)
    }
    // if(worldView) worldView.loadedChunks = {}
  }

  bot.viewer.focusPoint = (position) => {
    for (const socket of sockets) {
      socket.emit('focusPoint', position)
    }
  }

  io.on('connection', (socket) => {
    socket.emit('version', bot.version)
    sockets.push(socket)

    worldView = new WorldView(bot.world, viewDistance, bot.entity.position, socket)
    worldView.init(bot.entity.position)

    worldView.on('blockClicked', (block, face, button) => {
      bot.viewer.emit('blockClicked', block, face, button)
    })

    for (const id in primitives) {
      socket.emit('primitive', primitives[id])
    }

    function botPosition () {
      const packet = { pos: bot.entity.position, yaw: bot.entity.yaw, addMesh: true, entity: bot.entity }
      if (firstPerson) {
        packet.pitch = bot.entity.pitch
      }
      socket.emit('position', packet)
      worldView.updatePosition(bot.entity.position)
    }

    bot.on('move', botPosition)
    worldView.listenToBot(bot)
    socket.on('disconnect', () => {
      bot.removeListener('move', botPosition)
      worldView.removeListenersFromBot(bot)
      sockets.splice(sockets.indexOf(socket), 1)
    })
  })

  http.listen(port, () => {
    // console.log(`Prismarine viewer web server running on *:${port}`)
  })

  bot.viewer.close = () => {
    http.close()
    for (const socket of sockets) {
      socket.disconnect()
    }
  }
}
